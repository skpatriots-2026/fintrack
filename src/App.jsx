...
// ── localStorage adapter (replaces window.storage from preview runtime) ──────
const storage = {
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const ld = async (k,set) => { try { const r=await storage.get(k); if(r) set(JSON.parse(r.value)); } catch {} };
      await Promise.all([ld("ft_txns",setTxns),ld("ft_budgets",setBudgets),ld("ft_scores",setScores),ld("ft_goals",setGoals)]);
      try { const r=await storage.get("ft_theme"); if(r) setThemeKey(r.value); } catch {}
      setLoaded(true);
    })();
  }, []);

  const sv = async (k,data,setter) => { setter(data); try { await storage.set(k,JSON.stringify(data)); } catch {} };
  ...
  const selectTheme = async key => {
    ...
    try { await storage.set("ft_theme",key); } catch {}
  };
  ...
  const send=async()=>{
    ...
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:sys,messages:nm.map(m=>({role:m.role,content:m.content}))})
      });
